...
typedef enum {
	...
	GPIO_FLOAT		= 0,		//pin floating
	GPIO_PU			= 1,		//pull up
	GPIO_OD			= 0,		//open drain
	GPIO_PP			= 1,		//push-pull
	GPIO_NOEXTI		= 0,		//no EXTI
	GPIO_EXTI		= 1,		//EXTI on pin
	GPIO_Slow		= 0,		//slew, 2MHz
	GPIO_Fast		= 1			//10MHz
}
GPIO_Pin_TypeDef;
...
typedef enum {
	...
	CLK_DIV32,
	CLK_DIV64,
	CLK_DIV128	// 111 = div by 128
} CLK_CKDIVR_VAL_TypeDef;
...
/*
#define Bit(VAR,Place,Value)	   ((Value) ? \
                                   ((VAR) |= ((uint8_t)1<<(Place))) : \
                                   ((VAR) &= (((uint8_t)1<<(Place))^(uint8_t)255)))
*/
//redefined Bit to function like Bit(), but set/clear according to masks, which are already defined.
#define Bit(VAR,msk,Value)			((Value) ? \
									((VAR) |= ((uint8_t)(msk))) : \
									((VAR) &= ((~(uint8_t)(msk)))))
#define MskBits(byte,bitmsk,bitval)      ( (byte) = ((byte) & (~(bitmsk))) | ((bitval) & (bitmsk)) )
//#define MskBit(Dest,Msk,Src)      ( (Dest) = ((Msk) & (Src)) | ((~(Msk)) & (Dest)) )


#define ValBit(VAR,bitmsk)         ((uint8_t)(VAR) & (uint8_t)(bitmsk))
